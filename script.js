document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const FORM_SUCCESS_CLASS = 'form--success';
    const FORM_SENDING_CLASS = 'form--sending';

    function getFieldWrapper(field) {
      return field.closest('.form-group, .field-wrapper, [data-field]') || field.parentElement;
    }
    ...
    function showError(field, message) {
      ...
      let errorEl = wrapper.querySelector('.field-error-message');
      if (!errorEl) {
    ...
    function validateField(field) {
      ...
      const label = field.getAttribute('data-label') ||
        (contactForm.querySelector('label[for="' + field.id + '"]') || {}).textContent ||
        name;
      ...
      if (required && value === '') {
        showError(field, label ? label.trim() + ' is required.' : 'This field is required.');
        return false;
      }
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target.closest('input, textarea, select');
      if (!field) return;
      if (getFieldWrapper(field).classList.contains(FIELD_ERROR_CLASS)) {
        validateField(field);
      }
    });
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const fields = Array.from(contactForm.querySelectorAll('input, textarea, select'))
        .filter(function (f) {
          return f.type !== 'submit' && f.type !== 'button' && f.type !== 'reset' && f.type !== 'hidden';
        });
      ...
      contactForm.classList.add(FORM_SENDING_CLASS);
      const submitBtn = contactForm.querySelector('[type="submit"]');
      if (submitBtn) {
        submitBtn.disabled = true;
        submitBtn._originalText = submitBtn.textContent;
        submitBtn.textContent = submitBtn.getAttribute('data-sending-text') || 'Sending…';
      }

      // Simulate async submission — replace with real fetch/XHR as needed
      setTimeout(function () {
        contactForm.classList.remove(FORM_SENDING_CLASS);
        contactForm.classList.add(FORM_SUCCESS_CLASS);
        ...
        successMsg.textContent = contactForm.getAttribute('data-success-message') || 'Thank you! Your message has been sent.';

        if (submitBtn) {
          submitBtn.disabled = false;
          submitBtn.textContent = submitBtn._originalText || 'Send';
        }
        ...
        setTimeout(function () {
          contactForm.classList.remove(FORM_SUCCESS_CLASS);
  ...
  const ANIMATE_SELECTOR = '.fade-in, [data-animate], .animate-on-scroll';
  const VISIBLE_CLASS = 'is-visible';
  const ANIMATE_ONCE = true;

  const animatableElements = Array.from(document.querySelectorAll(ANIMATE_SELECTOR));
...
function initVariantSelection(product, t) {
  ...
  variantButtons.forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      if (variantId) {
        ...
        if (variantRowUnavailable(matchedVariant)) return;
  ...
  var groupArray = Array.from(variantGroups);
    ...
    // Select first in-stock option in the first group (skip qty=0 / unavailable)
    var firstBtn = groupArray[0].querySelector('.variant-option:not(.disabled)');
...
function updateVariantUI(variant, product, t, selectedAttributes) {
  ...
  if (variant) {
    ...
    // Update stock status
    const variantInStock = !variantRowUnavailable(variant);
    if (stockDisplay) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'ecdc127b-eac2-4e8c-a33c-8f16cc4974da',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'ecdc127b-eac2-4e8c-a33c-8f16cc4974da';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'ecdc127b-eac2-4e8c-a33c-8f16cc4974da';
